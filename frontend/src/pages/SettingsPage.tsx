...
export default function SettingsPage() {
  ...
  const saveCompany = async () => {
    ...
    setMessage("公司设置已保存"); setTimeout(() => setMessage(""), 3000); setSaving(false);
  };
  ...
  const saveRegistration = async () => {
    ...
    if (d.success) setMessage("注册设置已保存"); else setMessage("保存失败: " + (d.error || ""));
  ...
  const saveAI = async () => {
    ...
    const d = await r.json();
    if (d.success) setMessage("AI配置已保存"); else setMessage("保存失败: " + (d.error || ""));
    setTimeout(() => setMessage(""), 3000); setSaving(false);
  };
  ...
  const tabs = [
    { id: "company", label: "公司设置", icon: Building2 },
    ...(isAdmin ? [{ id: "modules", label: "模块管理", icon: LayoutGrid }] : []),
    { id: "ai", label: "AI大模型", icon: Cpu },
    ...(isAdmin ? [{ id: "users", label: "用户管理", icon: Users }] : []),
    ...(isSuperAdmin ? [{ id: "database", label: "数据库管理", icon: HardDrive }] : []),
  ];
  ...
  if (loading) return <div className="flex items-center justify-center h-64 text-text-muted"><Settings size={20} className="animate-spin mr-2" />加载中...</div>;
    ...
    <div className="h-full flex flex-col">
      <div className="flex items-center justify-between px-5 py-3 border-b border-border bg-bg-card">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-base font-bold text-text">系统设置</h1>
          <span className="text-[10px] px-2 py-1 rounded bg-primary/10 text-primary font-medium">{isSuperAdmin ? "超级管理员" : "管理员"}</span>
        </div>
        ...
        <div className="w-40 border-r border-border bg-bg-card p-2 space-y-1">
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id as any)}
              className={`w-full flex items-center gap-2 px-3 py-2 rounded text-xs transition-all ${tab === t.id ? "bg-primary text-white" : "text-text-muted hover:bg-bg hover:text-text"}`}>
        ...
        <div className="flex-1 overflow-auto p-5">
          {tab === "company" && (
            <div className="max-w-lg space-y-4">
              <h2 className="text-sm font-semibold text-text">公司信息</h2>
              <div><label className="block text-xs font-medium text-text mb-2">公司名称</label><input type="text" value={company.name} onChange={e => setCompany(prev => ({ ...prev, name: e.target.value }))} className={inputCls} /></div>
              <button onClick={saveCompany} disabled={saving} className={btnCls}><Save size={14} /> {saving ? "保存中..." : "保存设置"}</button>

              {isAdmin && (
                <>
                  <div className="border-t border-border pt-4 mt-4">
                    <h2 className="text-sm font-semibold text-text mb-3">用户注册设置</h2>
                    <div className="flex items-center justify-between p-3 bg-bg-card border border-border rounded">
                      <div>
                        <p className="text-xs font-medium text-text">开放用户注册</p>
                        <p className="text-[11px] text-text-muted mt-0.5">关闭后新用户将无法自行注册账号</p>
                      </div>
                      <button
                    ...
                    <button onClick={saveRegistration} disabled={saving} className={`${btnCls} mt-3`}>
                      <Save size={14} /> {saving ? "保存中..." : "保存注册设置"}
                    </button>
                  </div>
                </>
              )}
              ...
              <div className="mt-6 pt-6 border-t border-border">
                <div className="flex items-center justify-between p-3 bg-bg border border-border rounded-lg">
                  <div>
                    <div className="text-xs font-semibold text-text">🛡️ 空中模式（私有化策略）</div>
                    <div className="text-[10px] text-text-muted mt-0.5">
                      {airGapMode ? "已开启：仅允许白名单内模型端点，外部AI服务被拦截" : "已关闭：允许访问所有已配置的AI服务"}
                    </div>
                  </div>
                  <button
                    onClick={async () => {
                      ...
                      if (res.success) {
                        ...
                        setMessage(newMode ? "空中模式已开启" : "空中模式已关闭");
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowAddUser(false)}>
          <div className="bg-bg-card shadow-xl w-80 p-5 rounded" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div><label className="block text-xs font-medium text-text mb-1.5">邮箱</label><input type="email" placeholder="请输入邮箱" value={newUser.email} onChange={e => setNewUser(p => ({ ...p, email: e.target.value }))} className={inputCls} /></div>
              <div><label className="block text-xs font-medium text-text mb-1.5">昵称</label><input type="text" placeholder="请输入昵称" value={newUser.nickname} onChange={e => setNewUser(p => ({ ...p, nickname: e.target.value }))} className={inputCls} /></div>
              <div><label className="block text-xs font-medium text-text mb-1.5">密码</label><input type="password" placeholder="请输入密码" value={newUser.password} onChange={e => setNewUser(p => ({ ...p, password: e.target.value }))} className={inputCls} /></div>
              <div><label className="block text-xs font-medium text-text mb-1.5">角色</label>
                <select value={newUser.role} onChange={e => setNewUser(p => ({ ...p, role: e.target.value }))} className={inputCls}>
                  <option value="user">普通用户</option><option value="admin">管理员</option>{isSuperAdmin && <option value="super_admin">超级管理员</option>}
                </select>
              </div>
            </div>
            <div className="flex justify-end gap-2 mt-4">
              <button onClick={() => setShowAddUser(false)} className="px-3 py-2 text-xs text-text-muted hover:bg-bg rounded">取消</button>
              <button onClick={addUser} className="px-3 py-2 bg-primary text-white text-xs rounded font-medium hover:opacity-90">创建</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showAddConfig && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowAddConfig(false)}>
          <div className="bg-bg-card shadow-xl w-96 p-5 rounded" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-bold">新建配置版本</h2>
              <button onClick={() => setShowAddConfig(false)} className="text-text-muted hover:text-text p-1 rounded hover:bg-bg"><X size={16} /></button>
            </div>
            <div className="space-y-3">
              <div><label className="block text-xs font-medium text-text mb-1.5">配置类型</label>
                <select value={newConfig.config_type} onChange={e => setNewConfig(p => ({ ...p, config_type: e.target.value }))} className={inputCls}>
                  <option value="ai">AI配置</option><option value="system">系统配置</option><option value="ui">界面配置</option><option value="security">安全配置</option>
                </select>
              </div>
              <div><label className="block text-xs font-medium text-text mb-1.5">配置键</label><input type="text" placeholder="如 llm_api_key" value={newConfig.config_key} onChange={e => setNewConfig(p => ({ ...p, config_key: e.target.value }))} className={inputCls} /></div>
              <div><label className="block text-xs font-medium text-text mb-1.5">配置值</label><textarea placeholder="JSON格式" value={newConfig.config_value} onChange={e => setNewConfig(p => ({ ...p, config_value: e.target.value }))} rows={3} className={`${inputCls} font-mono text-[11px]`} /></div>
              <div><label className="block text-xs font-medium text-text mb-1.5">变更原因</label><input type="text" placeholder="可选" value={newConfig.change_reason} onChange={e => setNewConfig(p => ({ ...p, change_reason: e.target.value }))} className={inputCls} /></div>
            </div>
            <div className="flex justify-end gap-2 mt-4">
              <button onClick={() => setShowAddConfig(false)} className="px-3 py-2 text-xs text-text-muted hover:bg-bg rounded">取消</button>
              <button onClick={addConfigVersion} className="px-3 py-2 bg-primary text-white text-xs rounded font-medium hover:opacity-90">保存</button>
            </div>
          </div>
        </div>
      )}
...
function UserManagementTab() {
  ...
  async function deleteEmp(id: number, type: string) {
    if (type === "ai") { alert("AI员工不可删除，只能修改信息"); return; }
    if (!confirm("确定删除该员工？此操作不可撤销。")) return;
    try {
  ...
  if (loading) return <div className="text-text-muted text-xs py-8 text-center">加载中...</div>;
    ...
    <div className="max-w-4xl space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-semibold text-text">👤 人类员工 ({humans.length})</h2>
          <a href="/employees" className="text-xs text-primary hover:underline">+ 新增员工</a>
        </div>
        <div className="bg-bg-card border border-border rounded overflow-hidden">
          <table className="w-full">
            <thead><tr className="border-b border-border bg-bg">
              <th className="px-3 py-2 text-left text-[11px] font-medium text-text-muted">员工</th>
              <th className="px-3 py-2 text-left text-[11px] font-medium text-text-muted">邮箱</th>
              <th className="px-3 py-2 text-left text-[11px] font-medium text-text-muted">部门/岗位</th>
              <th className="px-3 py-2 text-left text-[11px] font-medium text-text-muted">技能</th>
              <th className="px-3 py-2 text-left text-[11px] font-medium text-text-muted">操作</th>
            </tr></thead>
            <tbody>
              {humans.map(e => (
                  ...
                  <td className="px-3 py-2">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-text-muted">{e.role || "未分配"}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-3 py-2 text-xs text-text-muted">{e.email || "-"}</td>
                  <td className="px-3 py-2 text-[11px] text-text-muted">{e.department_name || "未分配"} / {e.position_sequence || ""}</td>
                  <td className="px-3 py-2">
                  ...
                  <td className="px-3 py-2">
                    <div className="flex gap-1.5">
                      <button onClick={() => { setEditId(e.id); setEditForm({ name: e.name, role: e.role, skills: e.skills, description: e.description }); }}
                        className="text-[11px] text-primary hover:underline px-2 py-1">编辑</button>
                      <button onClick={() => deleteEmp(e.id, e.employee_type)}
                        className="text-[11px] text-red-500 hover:underline px-2 py-1">删除</button>
                    </div>
                  </td>
                </tr>
              ))}
              {humans.length === 0 && <tr><td colSpan={5} className="text-center py-6 text-xs text-text-muted">暂无人类员工</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-semibold text-text">🤖 AI员工 ({ais.length})</h2>
          <span className="text-[10px] text-text-muted">不可删除，仅可修改</span>
        </div>
        <div className="bg-bg-card border border-border rounded overflow-hidden">
          <table className="w-full">
            <thead><tr className="border-b border-border bg-bg">
              <th className="px-3 py-2 text-left text-[11px] font-medium text-text-muted">员工</th>
              <th className="px-3 py-2 text-left text-[11px] font-medium text-text-muted">智能体类型</th>
              <th className="px-3 py-2 text-left text-[11px] font-medium text-text-muted">部门/岗位</th>
              <th className="px-3 py-2 text-left text-[11px] font-medium text-text-muted">技能</th>
              <th className="px-3 py-2 text-left text-[11px] font-medium text-text-muted">操作</th>
            </tr></thead>
            <tbody>
              ...
              {ais.length === 0 && <tr><td colSpan={5} className="text-center py-6 text-xs text-text-muted">暂无AI员工</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setEditId(null)}>
          <div className="bg-bg-card border border-border rounded-2xl shadow-xl w-full max-w-md" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-5 py-3 border-t border-border flex justify-end gap-2">
              <button onClick={() => setEditId(null)} className="px-4 py-2 text-xs text-text-muted hover:bg-bg rounded">取消</button>
              <button onClick={saveEdit} className="px-4 py-2 text-xs bg-primary text-white rounded hover:bg-primary/90">保存</button>
            </div>
          </div>
        </div>
      )}
...
function LLMSettingsTab() {
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div>
        <h3 className="text-sm font-semibold text-text mb-3">快速配置 · 主流大模型</h3>
      ...
      <div className="bg-bg-card border border-border rounded-lg p-4 space-y-4">
        <h3 className="text-sm font-semibold text-text">自定义配置</h3>
        ...
        <div><label className="flex items-center justify-between text-[11px] font-medium text-text-muted mb-1"><span>API Key</span>{cfg.llm_api_key_configured === "true" && !cfg.llm_api_key && <em className="not-italic text-green-500">已配置，留空不修改</em>}</label>
        ...
        <div><label className="block text-[11px] font-medium text-text-muted mb-1">模型名称</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div><label className="block text-[11px] font-medium text-text-muted mb-1">AI自动回复</label>
            <select value={cfg.ai_reply_enabled || "true"} onChange={e => setCfg((p: any) => ({ ...p, ai_reply_enabled: e.target.value }))}
              className={inputCls}><option value="true">启用</option><option value="false">禁用</option></select></div>
          <div><label className="block text-[11px] font-medium text-text-muted mb-1">回复延迟(ms)</label>
...
function DatabaseManagementTab() {
  ...
  async function handleBackup() {
    ...
    try {
      ...
      if (j.success) {
        setMessage(`备份成功：${j.data.filename} (${j.data.size_human})`);
        loadInfo(); loadBackups();
      } else setMessage("备份失败: " + (j.error || ""));
    } catch { setMessage("备份请求失败"); }
    setBackingUp(false);
    setTimeout(() => setMessage(""), 5000);
  }

  async function handleRestore(filename: string) {
    if (!confirm(`⚠️ 确定要从 ${filename} 恢复数据库？\n\n当前数据库将被覆盖，建议先手动创建备份。`)) return;
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) {
        setMessage(`恢复成功！安全回滚点: ${j.data.safety_backup}。建议重启后端服务。`);
        loadInfo();
      } else setMessage("恢复失败: " + (j.error || ""));
    } catch { setMessage("恢复请求失败"); }
  ...
  if (loading) return <div className="text-text-muted text-xs py-8 text-center">加载数据库信息...</div>;
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="bg-bg-card border border-border rounded-xl p-4">
          <div className="flex items-center gap-2 text-text-muted mb-2"><HardDrive size={14} /><span className="text-[11px]">引擎</span></div>
          <div className="text-sm font-bold">{dbInfo?.dialect || "-"}</div>
          <div className="text-[10px] text-text-muted mt-1">SQLite (sql.js)</div>
        </div>
        <div className="bg-bg-card border border-border rounded-xl p-4">
          <div className="flex items-center gap-2 text-text-muted mb-2"><GitBranch size={14} /><span className="text-[11px]">数据表</span></div>
          <div className="text-sm font-bold">{dbInfo?.table_count || 0}</div>
          <div className="text-[10px] text-text-muted mt-1">{dbInfo?.total_rows || 0} 行记录</div>
        </div>
        <div className="bg-bg-card border border-border rounded-xl p-4">
          <div className="flex items-center gap-2 text-text-muted mb-2"><Upload size={14} /><span className="text-[11px]">文件大小</span></div>
        ...
        <div className="bg-bg-card border border-border rounded-xl p-4">
          <div className="flex items-center gap-2 text-text-muted mb-2"><Clock size={14} /><span className="text-[11px]">备份数量</span></div>
          <div className="text-sm font-bold">{dbInfo?.backups_count || 0}</div>
          <div className="text-[10px] text-text-muted mt-1">{dbInfo?.latest_backup ? new Date(dbInfo.latest_backup).toLocaleDateString("zh-CN") : "无"}</div>
        </div>
      </div>

      {/* 操作按钮区 */}
      <div className="bg-bg-card border border-border rounded-xl p-5 space-y-4">
        <h3 className="text-sm font-semibold text-text flex items-center gap-2"><Shield size={15} /><span>数据库操作</span></h3>
        ...
        <div className="flex gap-3">
          ...
          >
            <Download size={13} /> {backingUp ? "备份中..." : "立即备份"}
          </button>
          {message && (
            <span className={`text-[11px] px-3 py-1.5 rounded ${message.includes("失败") ? "bg-red-50 text-red-600" : "bg-green-50 text-green-700"} flex items-center gap-1`}>
              {message}
            </span>
          )}
        </div>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
          <p className="text-[11px] text-amber-700 flex items-start gap-2">
            ...
            <span><strong>安全提示：</strong>恢复操作会覆盖当前数据库，系统会自动在恢复前创建安全快照。生产环境建议先手动备份再执行恢复。</span>
          </p>
        </div>
      </div>
      ...
      <div className="bg-bg-card border border-border rounded-xl p-5 space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-text flex items-center gap-2"><History size={15} /><span>备份历史</span></h3>
        ...
        {backups.length === 0 ? (
          <div className="text-center py-8 text-text-muted text-xs">
            ...
            <p>暂无备份</p>
            <p className="mt-1">点击上方「立即备份」创建第一个备份</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead><tr className="border-b border-border text-text-muted">
                <th className="text-left py-2 px-3 font-medium">文件名</th>
                <th className="text-right py-2 px-3 font-medium">大小</th>
              ...
              <tbody>
                {backups.map((b, i) => (
                  <tr key={i} className="border-b border-border/50 hover:bg-bg transition-colors">
                    ...
                    <td className="py-2.5 px-3 text-text-muted whitespace-nowrap">{new Date(b.created_at || b.modified_at).toLocaleString("zh-CN")}</td>
                    <td className="py-2.5 px-3 text-right">
                      <div className="flex justify-end gap-1.5">
                        ...
                        >
                          {restoring && restoreTarget === b.filename ? "恢复中..." : "恢复"}
                        </button>
                        <button
      ...
      {dbInfo?.table_stats && Object.keys(dbInfo.table_stats).length > 0 && (
        <div className="bg-bg-card border border-border rounded-xl p-5">
          <h3 className="text-sm font-semibold text-text mb-3">表详情</h3>
