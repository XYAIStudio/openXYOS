  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (tab === "login") { await login(email, password); }
        ...
        if (!agreed) { setError("请先阅读并同意用户协议和隐私政策"); setLoading(false); return; }
    ...
    <div className="flex w-full min-h-screen">
      <div className="hidden lg:flex flex-[0_0_55%] text-white px-16 py-20 flex-col justify-center relative overflow-hidden"
        ...
        <div className="relative z-10 max-w-[500px]">
          ...
          <p className="text-base opacity-90 mb-12 leading-relaxed">AI赋能下的人机共融企业管理效能增强管理系统<br />一键部署专业AI团队，让每个企业都拥有AI赋能的组织能力</p>
          <div className="flex flex-col gap-4">
            {[
              { icon: "🤖", title: "AI人机共融团队", desc: "一键部署100+位专业AI员工，CEO/CTO/CFO等高管随时待命" },
              { icon: "📊", title: "全流程AaaS管理", desc: "任务协作、知识沉淀、Token统计、编排引擎一体化" },
              { icon: "🔒", title: "企业级安全", desc: "多租户隔离、数据加密、权限管控、API限流" },
            ].map((f, i) => (
      ...
      <div className="flex-1 flex items-center justify-center p-10 bg-bg">
        <div className="w-full max-w-[400px]">
          ...
          <div className={`flex items-center gap-3 bg-primary-bg border border-primary-light ${R} px-4 py-3 mb-5 text-[13px] text-text`}>
            ...
            <span>体验账号：<strong className="text-primary">demo@demo.com</strong></span>
            <button onClick={fillDemo} className={`ml-auto px-3.5 py-2 bg-primary text-white text-xs font-semibold ${R} hover:opacity-90 whitespace-nowrap`}>一键填入</button>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="mb-4">
              <label className="block text-[13px] font-medium mb-2 text-text">邮箱地址</label>
            ...
            {tab === "register" && (
              <div className="mb-4">
                <label className="block text-[13px] font-medium mb-2 text-text">昵称</label>
                <input type="text" value={nickname} onChange={e => setNickname(e.target.value)} placeholder="您的称呼"
                  className={`w-full px-3.5 py-2.5 border-[1.5px] border-border ${R} text-sm bg-bg-card text-text outline-none transition-all focus:border-primary`} />
              </div>
            )}
            <div className="mb-4">
              <label className="block text-[13px] font-medium mb-2 text-text">密码</label>
              <input type="password" value={password} onChange={e => setPassword(e.target.value)}
                placeholder={tab === "register" ? "至少6位密码" : "请输入密码"} required
                className={`w-full px-3.5 py-2.5 border-[1.5px] border-border ${R} text-sm bg-bg-card text-text outline-none transition-all focus:border-primary`} />
            </div>
            ...
            <label className="flex items-start gap-2 mb-4 cursor-pointer select-none">
              ...
              <span className="text-xs text-text-muted leading-relaxed">
                我已阅读并同意
                <Link to="/user-agreement" target="_blank" className="text-primary hover:underline mx-0.5">《用户服务协议》</Link>
                和
                <Link to="/privacy-policy" target="_blank" className="text-primary hover:underline mx-0.5">《隐私政策》</Link>
              </span>
            </label>
            <button type="submit" disabled={loading}
              className={`w-full py-3 ${R} text-[15px] font-semibold text-white transition-all disabled:opacity-85`}
              style={{ background: "linear-gradient(135deg, #10B981, #059669)" }}>
              {loading ? "处理中..." : tab === "login" ? "登 录" : "立即注册"}
            </button>
            {tab === "register" && <p className="text-center mt-3.5 text-[12px] text-primary font-semibold">🎉 注册即享14天专业版免费试用</p>}
          </form>
        </div>
      </div>
    </div>
