...
function Loading(){return <div className="h-40 grid place-items-center text-text-muted animate-pulse">加载中…</div>}
...
export default function OpenApp(){
 ...
 if(loading||(user&&!moduleError&&moduleTenantId!==user.tenant_id))return <div className="h-screen grid place-items-center bg-bg text-text-muted">加载中…</div>;
 if(user&&moduleError)return <div className="h-screen grid place-items-center bg-bg"><div className="text-center"><p>模块配置加载失败</p><p className="text-xs text-danger my-3">{moduleError}</p><button onClick={()=>void loadModules()} className="px-4 py-2 bg-primary text-white rounded">重新加载</button></div></div>;
