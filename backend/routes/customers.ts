...
customerRoutes.get("/:id", (req: AuthRequest, res) => {
  ...
  if (c) res.json({ success: true, data: c });
  else res.status(404).json({ success: false, error: "客户不存在" });
});
...
customerRoutes.get("/tickets/:id", (req: AuthRequest, res) => {
  ...
  if (t) res.json({ success: true, data: t });
  else res.status(404).json({ success: false, error: "工单不存在" });
});
...
customerRoutes.get("/bills/:id", (req: AuthRequest, res) => {
  ...
  if (b) res.json({ success: true, data: b });
  else res.status(404).json({ success: false, error: "账单不存在" });
});
