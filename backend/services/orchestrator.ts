...
// 创建编排任务
export function createOrchestration(title: string, description: string, goal: string, createdBy: number, tenantId: number): number {
  const result = dbRun(
    `INSERT INTO orchestration_tasks (title, description, goal, status, created_by, tenant_id)
     VALUES (?, ?, ?, 'pending', ?, ?)`,
    [title, description, goal, createdBy, tenantId]
  );
...
export async function analyzeTask(orchestrationId: number): Promise<any> {
  ...
  if (FEATURE_FLAGS.ENABLE_GOVERNANCE_ORCHESTRATION) {
    // 动态获取创建者的实际层级（从用户表获取 ai_level）
    const creator = dbGet("SELECT ai_level FROM users WHERE id = ? AND tenant_id = ?", [task.created_by, task.tenant_id]) as any;
    ...
    { role: "system", content: `你是任务分析专家。请分析以下任务，输出JSON格式：
{
  ...
  if (analysis.subtasks) {
    for (let i = 0; i < analysis.subtasks.length; i++) {
      ...
      dbRun(
        `INSERT INTO sub_tasks (orchestration_id, title, description, required_skills, depends_on, sort_order, tenant_id)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [orchestrationId, sub.title, sub.description, sub.required_skills || '', sub.depends_on || '', i, task.tenant_id]
      );
  ...
  for (const subtask of subtasks) {
    ...
    if (bestAgent) {
      ...
      if (isSensitiveAction(subtask.title, subtask.description) && FEATURE_FLAGS.ENABLE_HUMAN_IN_THE_LOOP) {
        ...
        assignments.push({ subtask: subtask.title, agent: bestAgent.name, score: bestScore.toFixed(2), requiresReview: true });
        // 暂不分配，等待人工审核
        continue;
      }

      dbRun("UPDATE sub_tasks SET assigned_to = ?, status = 'assigned' WHERE id = ?", [bestAgent.id, subtask.id]);
      assignments.push({ subtask: subtask.title, agent: bestAgent.name, score: bestScore.toFixed(2) });
    }
  }
