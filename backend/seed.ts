...
function requiredSeedSecret(name: "SEED_ADMIN_PASSWORD" | "SEED_DEMO_PASSWORD"): string {
  ...
  if (!value || value.length < 12) {
    throw new Error(`${name} must be at least 12 characters when SEED_DEMO_DATA=true`);
  }
  return value;
}
...
export function seedDatabase() {
  ...
  // Demo users, companies, departments and modules consistently belong to tenant #2.
  // Keep the tenant row present even when a prior partial seed left user data behind.
  dbRun(
    "INSERT OR IGNORE INTO tenants (id, name, slug, status, plan) VALUES (2, ?, ?, 'active', 'community')",
    ["openXYOS 演示租户", "openxyos-demo"],
  );
  ...
  if (userCount.c === 0) {
    ...
    const adminHash = bcrypt.hashSync(requiredSeedSecret("SEED_ADMIN_PASSWORD"), 10);
    dbRun("INSERT INTO users (email, password_hash, nickname, role, tenant_id) VALUES (?, ?, ?, ?, ?)",
      [seedAdminEmail(), adminHash, "超级管理员", "super_admin", 1]);

    const demoHash = bcrypt.hashSync(requiredSeedSecret("SEED_DEMO_PASSWORD"), 10);
    dbRun("INSERT INTO users (email, password_hash, nickname, role, tenant_id) VALUES (?, ?, ?, ?, ?)",
      ["demo@demo.com", demoHash, "张总", "admin", 2]);

    const userHash = bcrypt.hashSync(requiredSeedSecret("SEED_DEMO_PASSWORD"), 10);
    dbRun("INSERT INTO users (email, password_hash, nickname, role, tenant_id) VALUES (?, ?, ?, ?, ?)",
    ...
    const notes = [
      { title: "协作建议使用说明", content: "群聊中的智能助手可提供独立建议与纪要草稿；任务分配、批准和执行由组织成员确认。", tags: "协作,说明" },
      { title: "开源示例说明", content: "示例数据仅用于本地开发和功能演示，不代表真实客户、合同、价格或业务承诺。", tags: "开源,示例" },
    ];
...
function migrateDatabase() {
  ...
  const existingSuperAdmin = dbGet("SELECT id, role FROM users WHERE email = ?", [adminEmail]) as any;
  if (!existingSuperAdmin) {
    dbRun("INSERT INTO users (email, password_hash, nickname, role, tenant_id) VALUES (?, ?, ?, ?, ?)",
      [adminEmail, adminHash, "超级管理员", "super_admin", 1]);
    console.log(`[迁移] 已添加超级管理员 ${adminEmail}`);
  }
