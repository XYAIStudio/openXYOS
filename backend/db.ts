...
export async function initDatabase(): Promise<void> {
  ...
  // This migration is consumed by runtime services and must run for both new and existing databases.
  const auditMigration = fs.readFileSync(path.join(__dirname, "migrations/013_audit_bundle.sql"), "utf8");
  db.run(auditMigration);
  const clusterMigration = fs.readFileSync(path.join(__dirname, "migrations/014_cluster_nodes.sql"), "utf8");
  db.run(clusterMigration);
  const industryMigration = fs.readFileSync(path.join(__dirname, "migrations/015_tenant_industry_packages.sql"), "utf8");
  db.run(industryMigration);
  const moduleSettingsMigration = fs.readFileSync(path.join(__dirname, "migrations/016_tenant_module_settings.sql"), "utf8");
  db.run(moduleSettingsMigration);
  try {
    db.run("ALTER TABLE evidence_bundles ADD COLUMN content TEXT NOT NULL DEFAULT '{}'");
  } catch (error: any) {
...
function runMigrations() {
  ...
  try {
    ...
    // Older databases predate server-side token revocation. Add the counters in place.
    for (const statement of [
      "ALTER TABLE users ADD COLUMN token_version INTEGER NOT NULL DEFAULT 0",
      "ALTER TABLE tenants ADD COLUMN token_version INTEGER NOT NULL DEFAULT 0",
    ]) {
      try {
        db.run(statement);
      } catch (error: any) {
    ...
    // 通用治理基础：权限、通信规则、流程模板与审计事件。
    db.run(`CREATE TABLE IF NOT EXISTS governance_permission_matrix (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      tenant_id INTEGER NOT NULL,
    ...
    db.run(`CREATE TABLE IF NOT EXISTS governance_comm_rules (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      tenant_id INTEGER NOT NULL,
    ...
    db.run(`CREATE TABLE IF NOT EXISTS governance_process_templates (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      tenant_id INTEGER NOT NULL,
      name TEXT NOT NULL,
      description TEXT,
    ...
    db.run(`CREATE TABLE IF NOT EXISTS governance_event_log (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      tenant_id INTEGER NOT NULL,
      action_id TEXT,
      actor_type TEXT NOT NULL,
    ...
    try { db.run('CREATE INDEX IF NOT EXISTS idx_governance_permission_tenant ON governance_permission_matrix(tenant_id)'); } catch(e) {}
    try { db.run('CREATE INDEX IF NOT EXISTS idx_governance_comm_tenant ON governance_comm_rules(tenant_id)'); } catch(e) {}
    try { db.run('CREATE INDEX IF NOT EXISTS idx_governance_event_tenant ON governance_event_log(tenant_id, created_at)'); } catch(e) {}

    saveDb();
    console.log("[迁移] 通用治理表已就绪");
    ...
    try { db.run("CREATE INDEX IF NOT EXISTS idx_kf_external ON knowledge_files(tenant_id, external_id)"); } catch(e) {}
    try { db.run("CREATE INDEX IF NOT EXISTS idx_kn_external ON knowledge_notes(tenant_id, external_id)"); } catch(e) {}
  ...
  try {
    ...
    // 通用治理事件增加可选编排关联字段。
    try { db.run("ALTER TABLE governance_event_log ADD COLUMN orchestration_id INTEGER DEFAULT NULL"); } catch (e: any) {
      if (!e.message?.includes("duplicate column")) throw e;
    }
